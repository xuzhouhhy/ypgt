 ...
 * 时间 2016/6/6.
 */
@EFragment(R.layout.fragment_zdsb_bt)
public class TerminalNecessaryFragment extends BusinessFragment {

    @ViewById
    LinearLayout linearLayoutRoot;

    @ViewById
    BusinessScanEdit etZCBH;
    @ViewById
    ...
    public boolean logicCheck() {
        ...
        if(((RecordActivity)getActivity()).getController().isCreateRecord()) {
            DataSet dataSet = ((RecordActivity) getActivity()).getController().getCurrentDataSet();
            List<DataSet> dataSetList = dbManager.queryByCondition(dataSet, Enum.TERMINAL_FIELD_ZCBH, etZCBH.getControlValue(), false);
            if (dataSetList.size() > 0) {
                T.show(getContext(), getString(R.string.zdsb_exsit), Toast.LENGTH_SHORT);
                return false;
            }
        }
