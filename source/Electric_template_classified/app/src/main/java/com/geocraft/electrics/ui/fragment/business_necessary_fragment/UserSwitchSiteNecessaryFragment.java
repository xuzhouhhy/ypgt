 ...
 * 时间 2016/6/11.
 */
@EFragment(R.layout.fragment_yhkgz_bt)
public class UserSwitchSiteNecessaryFragment extends BusinessFragment {
	@ViewById
	...
	public boolean logicCheck() {
		...
		{
			DataSet dataSet = ((RecordActivity) getActivity()).getController().getCurrentDataSet();
			List<DataSet> dataSetList = dbManager.queryByCondition(dataSet, Enum.USERSWITCHSITE_FIELD_MC, etMC.getControlValue(), false);
			if (dataSetList.size() > 0) {
				T.show(getContext(), getString(R.string.yhkgz_exsit), Toast.LENGTH_SHORT);
				return false;
			}
		}
