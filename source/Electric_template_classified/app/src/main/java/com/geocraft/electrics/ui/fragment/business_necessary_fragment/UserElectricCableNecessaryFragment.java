 ...
 * 时间 2016/6/11.
 */
@EFragment(R.layout.fragment_yhdld_bt)
public class UserElectricCableNecessaryFragment extends BusinessFragment {
	@ViewById
	...
	public boolean logicCheck() {
		...
		{
			DataSet dataSet = ((RecordActivity) getActivity()).getController().getCurrentDataSet();
			List<DataSet> dataSetList = dbManager.queryByCondition(dataSet, Enum.USERELECTRICCABLE_FIELD_MC, etMC.getControlValue(), false);
			if (dataSetList.size() > 0) {
				T.show(getContext(), getString(R.string.yhdld_exsit), Toast.LENGTH_SHORT);
				return false;
			}
		}
