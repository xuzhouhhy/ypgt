 ...
 * 时间 2016/6/11.
 */
@EFragment(R.layout.fragment_yhpds_bt)
public class UserElectricityDistributionRoomNecessaryFragment extends BusinessFragment {
	@ViewById
		...
		if(((RecordActivity)getActivity()).getController().isCreateRecord()) {
			...
			if (primartyKey > 0) {
				...
				if (queyDataSet != null) {
					...
					etZCDW.setControlValue(strZCDW);
				}
			}
		}
	...
	public boolean logicCheck() {
		...
		{
			DataSet dataSet = ((RecordActivity) getActivity()).getController().getCurrentDataSet();
			List<DataSet> dataSetList = dbManager.queryByCondition(dataSet, Enum.USERTRANSFORMER_FIELD_MC, etMC.getControlValue(), false);
			if (dataSetList.size() > 0) {
				T.show(getContext(), getString(R.string.yhbyq_exsit), Toast.LENGTH_SHORT);
				return false;
			}
		}
