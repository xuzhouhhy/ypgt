 ...
 * 时间 2016/6/6.
 */
@EFragment(R.layout.fragment_byq_bt)
public class TransformerNWNecessaryFragment extends BusinessFragment {
    ...
    protected void init() {
        ...
        if(((RecordActivity)getActivity()).getController().isCreateRecord())
        {
            ...
            if(list.size() > 0)
            {
                etMC.setControlValue(list.get(list.size() - 1).GetFieldValueByName(Enum.TRANSFORMER_FIELD_MC));
            }
        }
    }
    ...
    public boolean logicCheck() {
        ...
        {
            DataSet dataSet = ((RecordActivity) getActivity()).getController().getCurrentDataSet();
            List<DataSet> dataSetList = dbManager.queryByCondition(dataSet, Enum.TRANSFORMER_FIELD_MC, etMC.getControlValue(), false);
            if (dataSetList.size() > 0) {
                T.show(getContext(), getString(R.string.byq_exsit), Toast.LENGTH_SHORT);
                return false;
            }
        }
